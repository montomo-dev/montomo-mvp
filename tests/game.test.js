...
test("配合表(SPECIAL_RESULTS)は30件に増え、レシピの重複キーがない", async () => {
  const { SPECIAL_RESULTS } = await import("../js/systems/breeding.js");
...
test("BreedingChartSceneは30件のレシピを4ページで表示できる", async () => {
  globalThis.document ??= {
    getElementById: () => ({
      style: {},
      classList: { add() {}, remove() {} },
  ...
  assert.equal(chart.recipes.length, 30);
  assert.equal(chart.totalPages, 4);
});
