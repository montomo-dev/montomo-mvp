...
test("MP導入前の旧セーブのなかまはロード時にMP・装備フィールドが補われる(スキルが永久に使えなくなる不具合の回帰防止)", async () => {
  const { migrateMonster } = await import("../js/systems/save.js");
  const { maxMpFor } = await import("../js/systems/growth.js");
  // 旧セーブのなかま: mp/maxMp/equippedを持たない
  const legacy = { uid: 1, speciesId: "mofuri", name: "モフリ", level: 42, exp: 0, maxHp: 100, hp: 100, atk: 30, def: 20, spd: 25, skills: ["happashusshu"] };
  migrateMonster(legacy);
  assert.equal(legacy.maxMp, maxMpFor(42));
  assert.equal(legacy.mp, legacy.maxMp);
  assert.equal(legacy.equipped, null);
  // 新しいセーブ(mp消費中・装備あり)は上書きされない
  const current = { level: 10, maxMp: 30, mp: 7, equipped: "leatherArmor" };
  migrateMonster(current);
  assert.equal(current.mp, 7);
  assert.equal(current.equipped, "leatherArmor");
});

test("MP回復アイテムがショップに並び、バトル中のどうぐ一覧には防具が出ない", async () => {
  assert.equal(ITEMS.magicWater.kind, "mp_heal");
  assert.ok(shopInventoryFor("sea_town1").includes("magicWater"));
  assert.ok(shopInventoryFor("factory_town1").includes("magicWaterEx"));
  ...
  const game = {
    ...
    ranch: [],
    items: { potionS: 1, leatherArmor: 2, magicWater: 1 },
    dex: { seen: [], caught: [] },
    lang: "ja",
    flags: {},
    save: () => true,
  ...
  const battle = new BattleScene(game, createMonster("dogura", 3), {});
  const listed = battle.ownedItemIds();
  assert.ok(listed.includes("potionS"));
  assert.ok(listed.includes("magicWater"));
  assert.ok(!listed.includes("leatherArmor"), "防具がバトル中のどうぐ一覧に出てしまっている");

  // マジックウォーターでMPが回復する
  battle.ally.mp = 3;
  battle.useItem("magicWater");
  assert.equal(battle.ally.mp, Math.min(battle.ally.maxMp, 3 + 25));
  assert.equal(game.items.magicWater, 0);
});
...
test("ショップの品揃えが多い街(全解禁済み)でも1ページ6件までにページ分割され、画面からはみ出さない", async () => {
  ...
  assert.equal(shop.itemIds.length, 15, "castle_town1の品揃え数が想定と違う");
  assert.equal(shop.totalPages, 3, "15品を1ページに収めようとして見切れる状態になっている");

  // 最後の商品(15番目)のカーソル位置での描画Y座標が画面(480px)に収まるか
