export const TILE_TYPES = {
  PLAIN: 0,
  BUSH: 1,
  TREE: 2,
  SPRING: 3,
  BOSS: 4,
  RANCH: 5,
  NEXT: 6,
  PREV: 7,
  SHOP: 8,
  HOUSE: 10,
  NPC: 11,
  INN: 12,
  TOWN_ENTER: 13,
  TOWN_EXIT: 14,
};

// レイアウト文字は36進数(0-9, a-z)で解釈する。0-9は既存タイルと完全互換。
export const TILE_CHARS = "0123456789abcdefghijklmnopqrstuvwxyz";

export function parseTileChar(ch) {
  const idx = TILE_CHARS.indexOf(ch);
  return idx === -1 ? 0 : idx;
}

export const START_STAGE_ID = "stage1";

export const STAGES = {
  stage1: {
    id: "stage1",
    shortName: "stage1",
    name: "はじまりのはら",
    nameEn: "Meadow of Beginnings",
    encounterRate: 0.12,
    wildLevels: [1, 3],
    nextStage: "stage2",
    townStage: "town1",
    spawns: {
      start: { x: 2, y: 10 },
      fromNext: { x: 13, y: 1 },
      fromTown: { x: 5, y: 9 },
    },
    palette: {
      ground: "#a9d977",
      accent: "#98cc66",
      bushFill: "#7bbb4d",
      bushStroke: "#5fa03e",
      treeLeaf: "#4e8f43",
      treeFruit: "#66a858",
    },
    layout: [
      "2222222222222222",
      "2500000000000362",
      "2020000200000002",
      "2021110220002202",
      "2001000000200002",
      "2000000000220002",
      "2002000000000102",
      "2002200011000002",
      "2000002001000002",
      "2000002200000002",
      "20000d0000000802",
      "2222222222222222",
    ],
    treasures: [{ id: "t1", money: 150, x: 8, y: 5, trap: false }],
    groundItems: [{ id: "i1", itemId: "potionM", x: 10, y: 3 }],
    rareSpecies: [{ speciesId: "tsukinone", level: 8 }],
  },
  town1: {
    id: "town1",
    shortName: "town1",
    name: "はじまりのまち",
    nameEn: "Starting Town",
    encounterRate: 0,
    townExitStage: "stage1",
    spawns: {
      start: { x: 8, y: 9 },
      fromField: { x: 8, y: 9 },
    },
    palette: {
      ground: "#d9c9a3",
      accent: "#c2b189",
      bushFill: "#7bbb4d",
      bushStroke: "#5fa03e",
      treeLeaf: "#4e8f43",
      treeFruit: "#66a858",
    },
    layout: [
      "2222222222222222",
      "2000000000000002",
      "20aa00aa000aa002",
      "20aa00aa000aa002",
      "20b000b0000c0082",
      "2000000000000002",
      "2000000050000002",
      "200000000b000002",
      "2000000000000002",
      "2000000000000002",
      "20000000e0000002",
      "2222222222222222",
    ],
    npcs: [
      { x: 2, y: 4, line: "ここは はじまりのまち。 やどやで つかれを いやせるよ。", lineEn: "This is the starting town. You can heal your fatigue at the inn." },
      { x: 6, y: 4, line: "ぼうけんの ちょうしは どうだい？ たまには まちで やすむのも いいものさ。", lineEn: "How's the adventure going? It's good to rest in town once in a while." },
      { x: 9, y: 7, line: "むらの おくには まだ みたことのない せかいが ひろがっているらしい…", lineEn: "Deep beyond the village, they say an unseen world stretches out..." },
    ],
  },
  stage2: {
    id: "stage2",
    shortName: "stage2",
    name: "こだまのしげみ",
    nameEn: "Echoing Thicket",
    encounterRate: 0.15,
    wildLevels: [3, 6],
    prevStage: "stage1",
    nextStage: "cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#8ec26b",
      accent: "#79ae57",
      bushFill: "#5d9842",
      bushStroke: "#447630",
      treeLeaf: "#346b34",
      treeFruit: "#4f8e46",
    },
    layout: [
      "2222222222222222",
      "2700001000000062",
      "2010222011000002",
      "2000200000111002",
      "2011000200000002",
      "2002000220000102",
      "2002010000030002",
      "2002200001100202",
      "2000102200002202",
      "2000000200000002",
      "2100000000100002",
      "2222222222222222",
    ],
    treasures: [{ id: "t2", money: 200, x: 6, y: 6, trap: false }],
    groundItems: [{ id: "i2", itemId: "bait", x: 8, y: 8 }],
    rareSpecies: [{ speciesId: "honbori", level: 10 }],
  },
  cave: {
    id: "cave",
    shortName: "どうくつ",
    name: "まよいのどうくつ",
    nameEn: "Cave of Lost Ways",
    encounterRate: 0.16,
    wildLevels: [5, 7],
    prevStage: "stage2",
    nextStage: "stage3",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#4a4658",
      accent: "#5c5870",
      bushFill: "#38344a",
      bushStroke: "#6a5f95",
      treeLeaf: "#6b6478",
      treeFruit: "#a996e8",
    },
    layout: [
      "2222222222222222",
      "2700000000000062",
      "2000220022000002",
      "2000220020122012",
      "2000000000020002",
      "2002200220000002",
      "2002210200000002",
      "2000000000002202",
      "2000002200002002",
      "2000102001100002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t3", money: 250, x: 7, y: 4, trap: true }],
    groundItems: [{ id: "i3", itemId: "potionM", x: 10, y: 7 }],
    rareSpecies: [{ speciesId: "tsukinone", level: 12 }],
  },
  stage3: {
    id: "stage3",
    shortName: "stage3",
    name: "ヌシのひろば",
    nameEn: "Lord's Clearing",
    nameEn: "Lord's Clearing",
    encounterRate: 0.18,
    wildLevels: [6, 9],
    prevStage: "cave",
    nextStage: "reverse_stage1",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#89b96b",
      accent: "#719c55",
      bushFill: "#587b3d",
      bushStroke: "#3c5628",
      treeLeaf: "#2f552b",
      treeFruit: "#56783d",
    },
    layout: [
      "2222222222222222",
      "2700000000000002",
      "2002200110000002",
      "2002100002200202",
      "2000012202000002",
      "2022002000000002",
      "2020000000030002",
      "2000000010000002",
      "2100220011000002",
      "2000200000000002",
      "2000000000000402",
      "2222222222222222",
    ],
    bossId: "nushi",
    bossLevel: 15,
    treasures: [{ id: "t4", money: 300, x: 5, y: 6, trap: true }],
    groundItems: [{ id: "i4", itemId: "atkSeed", x: 13, y: 4 }],
    rareSpecies: [{ speciesId: "tsukinone", level: 14 }],
  },
  reverse_stage1: {
    id: "reverse_stage1",
    shortName: "ure1",
    name: "ウラのはじまり",
    nameEn: "Reverse World Threshold",
    encounterRate: 0.16,
    wildLevels: [15, 18],
    wildSpecies: ["shizukudama", "kagerouzu", "pukurin", "hoshimogu", "shizukuya", "tsuyuhika"],
    prevStage: "stage3",
    nextStage: "reverse_stage2",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#3a3a3a",
      accent: "#4a4a4a",
      bushFill: "#2a2a3a",
      bushStroke: "#5a5a6a",
      treeLeaf: "#1a2a2a",
      treeFruit: "#6a5a8a",
    },
    layout: [
      "2222222222222222",
      "2000000000000062",
      "2002000000100002",
      "2002202202200002",
      "2000000002000202",
      "2000022000100202",
      "2010020000030002",
      "2010000000000002",
      "2000220010000002",
      "2000200000000002",
      "2000010000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t5", money: 400, x: 8, y: 6, trap: true }],
    groundItems: [{ id: "i5", itemId: "defSeed", x: 11, y: 4 }],
    rareSpecies: [{ speciesId: "honbori", level: 16 }],
  },
  reverse_stage2: {
    id: "reverse_stage2",
    shortName: "ure2",
    name: "ウラのしげみ",
    nameEn: "Reverse Thicket",
    encounterRate: 0.17,
    wildLevels: [18, 20],
    wildSpecies: ["kageuri", "nejiko", "moriame", "tsukihane"],
    prevStage: "reverse_stage1",
    nextStage: "reverse_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#2a3a2a",
      accent: "#3a4a3a",
      bushFill: "#1a2a1a",
      bushStroke: "#4a5a4a",
      treeLeaf: "#0a1a0a",
      treeFruit: "#5a4a6a",
    },
    layout: [
      "2222222222222222",
      "2700100000000062",
      "2002000022000002",
      "2002011002002202",
      "2002000000002002",
      "2012000000000002",
      "2000021000031102",
      "2000022000220002",
      "2011100010020002",
      "2000001220000002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t6", money: 450, x: 7, y: 7, trap: true }],
    groundItems: [{ id: "i6", itemId: "premiumBait", x: 9, y: 4 }],
    rareSpecies: [{ speciesId: "tsukinone", level: 18 }],
  },
  reverse_cave: {
    id: "reverse_cave",
    shortName: "うるどうくつ",
    name: "ウラのどうくつ",
    nameEn: "Reverse Cave",
    encounterRate: 0.18,
    wildLevels: [20, 22],
    wildSpecies: ["fuyudama", "kageuri", "hikariame", "sumiremo"],
    prevStage: "reverse_stage2",
    nextStage: "reverse_stage3",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#2a2a3a",
      accent: "#3a3a4a",
      bushFill: "#1a1a2a",
      bushStroke: "#4a4a5a",
      treeLeaf: "#4a3a5a",
      treeFruit: "#8a6aaa",
    },
    layout: [
      "2222222222222222",
      "2700000000000062",
      "2002002002200002",
      "2002202202000002",
      "2000000000002002",
      "2020020000002202",
      "2022022001000002",
      "2000000000000002",
      "2000110201022002",
      "2000000220002002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t7", money: 500, x: 8, y: 7, trap: true }],
    groundItems: [{ id: "i7", itemId: "atkSeed", x: 10, y: 8 }],
    rareSpecies: [{ speciesId: "honbori", level: 20 }],
  },
  reverse_stage3: {
    id: "reverse_stage3",
    shortName: "ure3",
    name: "ウラのヌシのひろば",
    nameEn: "Reverse Lord's Clearing",
    encounterRate: 0.18,
    wildLevels: [22, 25],
    wildSpecies: ["shizukudama", "kagerouzu", "pukurin", "kageuri", "hoshimogu", "fuyudama", "nejiko", "shizukuya", "tsuyuhika", "moriame", "tsukihane", "hikariame", "sumiremo"],
    prevStage: "reverse_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#3a2a4a",
      accent: "#4a3a5a",
      bushFill: "#2a1a3a",
      bushStroke: "#5a4a6a",
      treeLeaf: "#1a0a2a",
      treeFruit: "#6a4a8a",
    },
    layout: [
      "2222222222222222",
      "2700100000000002",
      "2002002200000002",
      "2002200000200002",
      "2000000022200202",
      "2010000020100202",
      "2010000000030002",
      "2000200000000002",
      "2000220100000002",
      "2000000000000002",
      "2000000010000402",
      "2222222222222222",
    ],
    bossId: "reverseNushi",
    bossLevel: 30,
    treasures: [{ id: "t8", money: 600, x: 6, y: 7, trap: true }],
    groundItems: [{ id: "i8", itemId: "defSeed", x: 12, y: 5 }],
    rareSpecies: [{ speciesId: "tsukinone", level: 25 }],
  },
  sea_stage1: {
    id: "sea_stage1",
    shortName: "umi1",
    name: "なぎさのいりぐち",
    nameEn: "Shoreline Entrance",
    encounterRate: 0.16,
    wildLevels: [25, 28],
    wildSpecies: ["kamomeru", "kaigaramaru", "kaigaran", "awairuka", "sazanami", "mizuhane"],
    prevStage: "reverse_stage3",
    nextStage: "sea_stage2",
    townStage: "sea_town1",
    spawns: {
      start: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      fromTown: { x: 4, y: 9 },
    },
    palette: {
      ground: "#3aa0c9",
      accent: "#4ab0d9",
      bushFill: "#2a8ab0",
      bushStroke: "#6ad0e8",
      treeLeaf: "#1a7a9a",
      treeFruit: "#e8d060",
    },
    layout: [
      "2222222222222222",
      "2000000000000062",
      "2002200002200002",
      "2010202200002202",
      "2000002011000002",
      "2010000000000002",
      "2000020001030002",
      "2000022000000002",
      "2000000220000002",
      "2000000000000002",
      "2001d00000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t9", money: 700, x: 8, y: 6, trap: true }],
    groundItems: [{ id: "i9", itemId: "premiumBait", x: 11, y: 4 }],
  },
  sea_town1: {
    id: "sea_town1",
    shortName: "umiMachi",
    name: "なぎさのまち",
    nameEn: "Seaside Town",
    encounterRate: 0,
    townExitStage: "sea_stage1",
    spawns: {
      start: { x: 8, y: 9 },
      fromField: { x: 8, y: 9 },
    },
    palette: {
      ground: "#8fd0e0",
      accent: "#a0dcea",
      bushFill: "#2a8ab0",
      bushStroke: "#6ad0e8",
      treeLeaf: "#1a7a9a",
      treeFruit: "#e8d060",
    },
    layout: [
      "2222222222222222",
      "2000000000000002",
      "20aa00aa000aa002",
      "20aa00aa000aa002",
      "20b000b0000c0082",
      "2000000000000002",
      "2000000050000002",
      "200000000b000002",
      "2000000000000002",
      "2000000000000002",
      "20000000e0000002",
      "2222222222222222",
    ],
    npcs: [
      { x: 2, y: 4, line: "なみおとが きもちいい まちだろう。やどやで ひとやすみ していきな。", lineEn: "Nice sound of the waves, isn't it? Take a rest at the inn." },
      { x: 6, y: 4, line: "うみの おくには まだ だれも みたことのない いきものが いるらしい。", lineEn: "Deep in the sea, they say there are creatures no one has ever seen." },
      { x: 9, y: 7, line: "しんかいヌシに いどむ まえには、ここで じゅんびを ととのえるといい。", lineEn: "Before challenging the Deep Sea Nushi, you should prepare here." },
    ],
  },
  sea_stage2: {
    id: "sea_stage2",
    shortName: "umi2",
    name: "さんごのうみ",
    nameEn: "Coral Sea",
    encounterRate: 0.17,
    wildLevels: [27, 30],
    wildSpecies: ["unizoku", "awairuka", "hikariebi", "mizugoma", "mizukusa"],
    prevStage: "sea_stage1",
    nextStage: "sea_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#2a8ab0",
      accent: "#3a9ac0",
      bushFill: "#1a7098",
      bushStroke: "#5ad0e8",
      treeLeaf: "#0a6088",
      treeFruit: "#f0d060",
    },
    layout: [
      "2222222222222222",
      "2700000000001062",
      "2000200110000002",
      "2010200001000002",
      "2001222000222002",
      "2010002000002202",
      "2022000010030002",
      "2002000022000002",
      "2000000002001102",
      "2000022000000002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t10", money: 800, x: 7, y: 7, trap: true }],
    groundItems: [{ id: "i10", itemId: "premiumBait", x: 9, y: 4 }],
  },
  sea_cave: {
    id: "sea_cave",
    shortName: "かいてい",
    name: "しんかいのどうくつ",
    nameEn: "Deep Sea Cave",
    encounterRate: 0.18,
    wildLevels: [29, 32],
    wildSpecies: ["hikariebi", "yamiankou", "mizutama", "shizumegane"],
    prevStage: "sea_stage2",
    nextStage: "sea_stage3",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#1a2a4a",
      accent: "#2a3a5a",
      bushFill: "#0a1a3a",
      bushStroke: "#3a5a8a",
      treeLeaf: "#241c38",
      treeFruit: "#6a5aaa",
    },
    layout: [
      "2222222222222222",
      "2700000000000062",
      "2000220220000012",
      "2000020200220102",
      "2022000000020002",
      "2002000010000002",
      "2000022002000002",
      "2000002002202202",
      "2022000000000002",
      "2002000100000002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t11", money: 900, x: 8, y: 7, trap: true }],
    groundItems: [{ id: "i11", itemId: "atkSeed", x: 10, y: 8 }],
  },
  sea_stage3: {
    id: "sea_stage3",
    shortName: "umi3",
    name: "シンカイヌシのひろば",
    nameEn: "Deep Sea Lord's Clearing",
    encounterRate: 0.18,
    wildLevels: [30, 33],
    wildSpecies: ["unizoku", "kamomeru", "kaigaramaru", "kaigaran", "awairuka", "hikariebi", "yamiankou", "sazanami", "mizuhane", "mizugoma", "mizukusa", "mizutama", "shizumegane", "haribune"],
    prevStage: "sea_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#1a4a68",
      accent: "#2a5a78",
      bushFill: "#0a3a58",
      bushStroke: "#4a8aa8",
      treeLeaf: "#0a2a48",
      treeFruit: "#8ac0d8",
    },
    layout: [
      "2222222222222222",
      "2700000000000002",
      "2002200002200002",
      "2000202200002102",
      "2010002000002202",
      "2010000010000002",
      "2000220001030002",
      "2000200000000002",
      "2000000000000002",
      "2000000220000002",
      "2000000000001402",
      "2222222222222222",
    ],
    bossId: "seaNushi",
    bossLevel: 35,
    treasures: [{ id: "t12", money: 1000, x: 6, y: 7, trap: true }],
    groundItems: [{ id: "i12", itemId: "defSeed", x: 12, y: 5 }],
  },
  snow_stage1: {
    id: "snow_stage1",
    shortName: "yuki1",
    name: "ゆきのおおどおり",
    nameEn: "Snowy Boulevard",
    encounterRate: 0.16,
    wildLevels: [33, 36],
    wildSpecies: ["shirokitsune", "kooridama", "yukimaro", "kooritsumu", "yukigamo"],
    prevStage: "sea_stage3",
    nextStage: "snow_stage2",
    townStage: "snow_town1",
    spawns: {
      start: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      fromTown: { x: 4, y: 9 },
    },
    palette: {
      ground: "#eaf4fb",
      accent: "#dcedf7",
      bushFill: "#c9e4f2",
      bushStroke: "#a8c8d8",
      treeLeaf: "#8fb8d0",
      treeFruit: "#f0a8c0",
    },
    layout: [
      "2222222222222222",
      "2000000000000062",
      "2010022000002202",
      "2002002000000002",
      "2002001022000002",
      "2000000002000002",
      "2000000000030002",
      "2001100001000002",
      "2000002202200002",
      "2000002000000002",
      "2000d00000100002",
      "2222222222222222",
    ],
    treasures: [{ id: "t13", money: 1100, x: 8, y: 6, trap: true }],
    groundItems: [{ id: "i13", itemId: "premiumBait", x: 11, y: 4 }],
  },
  snow_town1: {
    id: "snow_town1",
    shortName: "yukiMachi",
    name: "ゆきぐにのまち",
    nameEn: "Snowbound Town",
    encounterRate: 0,
    townExitStage: "snow_stage1",
    spawns: {
      start: { x: 8, y: 9 },
      fromField: { x: 8, y: 9 },
    },
    palette: {
      ground: "#dcedf7",
      accent: "#cde3f0",
      bushFill: "#c9e4f2",
      bushStroke: "#a8c8d8",
      treeLeaf: "#8fb8d0",
      treeFruit: "#f0a8c0",
    },
    layout: [
      "2222222222222222",
      "2000000000000002",
      "20aa00aa000aa002",
      "20aa00aa000aa002",
      "20b000b0000c0082",
      "2000000000000002",
      "2000000050000002",
      "200000000b000002",
      "2000000000000002",
      "2000000000000002",
      "20000000e0000002",
      "2222222222222222",
    ],
    npcs: [
      { x: 2, y: 4, line: "さむいだろう。やどやの だんろで あたたまっていくといい。", lineEn: "Must be cold, huh? Warm yourself by the fireplace at the inn." },
      { x: 6, y: 4, line: "ゆきの おくに すむ ヌシに ちかづく ときは きを つけて。", lineEn: "Be careful when approaching the Nushi that lives deep in the snow." },
      { x: 9, y: 7, line: "ゆきどけの きせつには この まちも にぎわうんだ。", lineEn: "This town gets lively when the snow melts each season." },
    ],
  },
  snow_stage2: {
    id: "snow_stage2",
    shortName: "yuki2",
    name: "こおりのみずうみ",
    nameEn: "Frozen Lake",
    encounterRate: 0.17,
    wildLevels: [35, 38],
    wildSpecies: ["kooritsumu", "pengiri"],
    prevStage: "snow_stage1",
    nextStage: "snow_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#dcedf7",
      accent: "#c9e4f2",
      bushFill: "#a8c8d8",
      bushStroke: "#eaf4fb",
      treeLeaf: "#7098b8",
      treeFruit: "#f0a8c0",
    },
    layout: [
      "2222222222222222",
      "2700000001000062",
      "2001110002200002",
      "2000002010201102",
      "2020002200000002",
      "2022000000000002",
      "2000000000030002",
      "2001220000002012",
      "2000020022012202",
      "2000000120000002",
      "2000000000001002",
      "2222222222222222",
    ],
    treasures: [{ id: "t14", money: 1200, x: 7, y: 7, trap: true }],
    groundItems: [{ id: "i14", itemId: "premiumBait", x: 9, y: 4 }],
  },
  snow_cave: {
    id: "snow_cave",
    shortName: "ひょうけつ",
    name: "ひょうけつのどうくつ",
    nameEn: "Frost Cave",
    encounterRate: 0.18,
    wildLevels: [37, 40],
    wildSpecies: ["pengiri", "yukibouzu"],
    prevStage: "snow_stage2",
    nextStage: "snow_stage3",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#c9e4f2",
      accent: "#a8c8d8",
      bushFill: "#8fb8d0",
      bushStroke: "#eaf4fb",
      treeLeaf: "#5a86a8",
      treeFruit: "#dcedf7",
    },
    layout: [
      "2222222222222222",
      "2700000000000062",
      "2002200200220002",
      "2010220220200002",
      "2000000000000002",
      "2000022000002202",
      "2020020000002002",
      "2022000002000002",
      "2000000002000002",
      "2000100000001002",
      "2000000000010002",
      "2222222222222222",
    ],
    treasures: [{ id: "t15", money: 1300, x: 8, y: 7, trap: true }],
    groundItems: [{ id: "i15", itemId: "atkSeed", x: 10, y: 8 }],
  },
  snow_stage3: {
    id: "snow_stage3",
    shortName: "yuki3",
    name: "ヒョウガヌシのひろば",
    nameEn: "Glacier Lord's Clearing",
    encounterRate: 0.18,
    wildLevels: [38, 41],
    wildSpecies: ["shirokitsune", "kooridama", "yukimaro", "kooritsumu", "pengiri", "yukibouzu", "yukigamo"],
    prevStage: "snow_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#f5fafd",
      accent: "#eaf4fb",
      bushFill: "#dcedf7",
      bushStroke: "#a8c8d8",
      treeLeaf: "#7098b8",
      treeFruit: "#f0a8c0",
    },
    layout: [
      "2222222222222222",
      "2700000000000002",
      "2010002200002002",
      "2002202000000202",
      "2000000002200002",
      "2000000002100002",
      "2000110000030002",
      "2000000000001002",
      "2002200022000002",
      "2002000000000002",
      "2000000010000402",
      "2222222222222222",
    ],
    bossId: "hyougaNushi",
    bossLevel: 40,
    treasures: [{ id: "t16", money: 1400, x: 6, y: 7, trap: true }],
    groundItems: [{ id: "i16", itemId: "defSeed", x: 12, y: 5 }],
  },
  desert_stage1: {
    id: "desert_stage1",
    shortName: "sabaku1",
    name: "すなのおおどおり",
    nameEn: "Desert Boulevard",
    encounterRate: 0.16,
    wildLevels: [40, 43],
    wildSpecies: ["saboko", "sunatokage", "sabotenko", "sunasasori", "sunaboko", "sunobori"],
    prevStage: "snow_stage3",
    nextStage: "desert_stage2",
    townStage: "desert_town1",
    spawns: {
      start: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      fromTown: { x: 4, y: 9 },
    },
    palette: {
      ground: "#e8c67a",
      accent: "#dcb862",
      bushFill: "#c9a24a",
      bushStroke: "#a8823a",
      treeLeaf: "#8a9a4a",
      treeFruit: "#c4d060",
    },
    layout: [
      "2222222222222222",
      "2000000000000062",
      "2002000010100002",
      "2002000000001102",
      "2001022002200002",
      "2000020000000002",
      "2020000000030002",
      "2022000000220002",
      "2000000220000002",
      "2000000200000002",
      "2000d10000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t17", money: 1500, x: 8, y: 6, trap: true }],
    groundItems: [{ id: "i17", itemId: "premiumBait", x: 11, y: 4 }],
  },
  desert_town1: {
    id: "desert_town1",
    shortName: "sabakuMachi",
    name: "オアシスのまち",
    nameEn: "Oasis Town",
    encounterRate: 0,
    townExitStage: "desert_stage1",
    spawns: {
      start: { x: 8, y: 9 },
      fromField: { x: 8, y: 9 },
    },
    palette: {
      ground: "#e8c67a",
      accent: "#dcb862",
      bushFill: "#c9a24a",
      bushStroke: "#a8823a",
      treeLeaf: "#8a9a4a",
      treeFruit: "#c4d060",
    },
    layout: [
      "2222222222222222",
      "2000000000000002",
      "20aa00aa000aa002",
      "20aa00aa000aa002",
      "20b000b0000c0082",
      "2000000000000002",
      "2000000050000002",
      "200000000b000002",
      "2000000000000002",
      "2000000000000002",
      "20000000e0000002",
      "2222222222222222",
    ],
    npcs: [
      { x: 2, y: 4, line: "オアシスの みずは きちょうだ。やどやで のどを うるおしていけ。", lineEn: "The oasis water is precious. Quench your thirst at the inn." },
      { x: 6, y: 4, line: "すなあらしの まえには かならず ここに たちよるんだ。", lineEn: "Always stop by here before a sandstorm hits." },
      { x: 9, y: 7, line: "さばくの おくには こだいの いせきが ねむっているとか…", lineEn: "Deep in the desert, an ancient ruin is said to be sleeping..." },
    ],
  },
  desert_stage2: {
    id: "desert_stage2",
    shortName: "sabaku2",
    name: "きょだいサボテンのたに",
    nameEn: "Giant Cactus Valley",
    encounterRate: 0.17,
    wildLevels: [42, 45],
    wildSpecies: ["rakudako", "sunasasori", "rakudan", "sunamaru"],
    prevStage: "desert_stage1",
    nextStage: "desert_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#dcb862",
      accent: "#c9a24a",
      bushFill: "#a8823a",
      bushStroke: "#e8c67a",
      treeLeaf: "#7a8a3a",
      treeFruit: "#c4d060",
    },
    layout: [
      "2222222222222222",
      "2700000100000062",
      "2000000000002202",
      "2012200022000202",
      "2000000000010002",
      "2000022000110002",
      "2000022000030002",
      "2000000002000002",
      "2002210022000002",
      "2000200000001102",
      "2000011000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t18", money: 1600, x: 7, y: 7, trap: true }],
    groundItems: [{ id: "i18", itemId: "premiumBait", x: 9, y: 4 }],
  },
  desert_cave: {
    id: "desert_cave",
    shortName: "いせき",
    name: "きゅうでんのいせき",
    nameEn: "Ruined Palace",
    encounterRate: 0.18,
    wildLevels: [44, 47],
    wildSpecies: ["rakudan", "sunamiira", "ishimaru"],
    prevStage: "desert_stage2",
    nextStage: "desert_stage3",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#c9a24a",
      accent: "#b8923a",
      bushFill: "#a8823a",
      bushStroke: "#dcb862",
      treeLeaf: "#6b5122",
      treeFruit: "#e8c67a",
    },
    layout: [
      "2222222222222222",
      "2700000001000062",
      "2000002200002002",
      "2002202002002202",
      "2000100002200002",
      "2020000000000002",
      "2022000200000002",
      "2000220200020102",
      "2000020000022002",
      "2010000000000002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t19", money: 1700, x: 8, y: 7, trap: true }],
    groundItems: [{ id: "i19", itemId: "atkSeed", x: 10, y: 8 }],
  },
  desert_stage3: {
    id: "desert_stage3",
    shortName: "sabaku3",
    name: "スナヌシのひろば",
    nameEn: "Sand Lord's Clearing",
    encounterRate: 0.18,
    wildLevels: [45, 48],
    wildSpecies: ["rakudako", "saboko", "sunatokage", "sabotenko", "sunasasori", "rakudan", "sunamiira", "sunaboko", "sunobori", "sunamaru", "ishimaru"],
    prevStage: "desert_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#f0d68a",
      accent: "#e8c67a",
      bushFill: "#dcb862",
      bushStroke: "#c9a24a",
      treeLeaf: "#8a9a4a",
      treeFruit: "#c4d060",
    },
    layout: [
      "2222222222222222",
      "2700000100000002",
      "2002000011000002",
      "2002000000001002",
      "2000022002200002",
      "2000020000000002",
      "2020000000030002",
      "2022000000220002",
      "2000002200000002",
      "2000001200001002",
      "2000000000000402",
      "2222222222222222",
    ],
    bossId: "sunaNushi",
    bossLevel: 45,
    treasures: [{ id: "t20", money: 1800, x: 6, y: 7, trap: true }],
    groundItems: [{ id: "i20", itemId: "defSeed", x: 12, y: 5 }],
  },
  factory_stage1: {
    id: "factory_stage1",
    shortName: "koujou1",
    name: "はいこうじょうのにゅうこう",
    nameEn: "Factory Entrance",
    encounterRate: 0.16,
    wildLevels: [47, 50],
    wildSpecies: ["kandenkko", "nejimaru", "hagurumaru", "sparkun"],
    prevStage: "desert_stage3",
    nextStage: "factory_stage2",
    townStage: "factory_town1",
    spawns: {
      start: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      fromTown: { x: 4, y: 9 },
    },
    palette: {
      ground: "#7a7a8a",
      accent: "#8a8a9a",
      bushFill: "#5a5a6a",
      bushStroke: "#4a4a56",
      treeLeaf: "#3a3a48",
      treeFruit: "#f0d040",
    },
    layout: [
      "2222222222222222",
      "2000000000000062",
      "2102022000002002",
      "2002000020002202",
      "2102000022000002",
      "2000000000000002",
      "2000022000030002",
      "2000020002000002",
      "2010000002000002",
      "2000000100000002",
      "2000d00000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t21", money: 1900, x: 8, y: 6, trap: true }],
    groundItems: [{ id: "i21", itemId: "premiumBait", x: 11, y: 4 }],
  },
  factory_town1: {
    id: "factory_town1",
    shortName: "koujouMachi",
    name: "はいきのまち",
    nameEn: "Scrapyard Town",
    encounterRate: 0,
    townExitStage: "factory_stage1",
    spawns: {
      start: { x: 8, y: 9 },
      fromField: { x: 8, y: 9 },
    },
    palette: {
      ground: "#8a8a94",
      accent: "#9a9aa4",
      bushFill: "#5a5a6a",
      bushStroke: "#4a4a56",
      treeLeaf: "#3a3a48",
      treeFruit: "#f0d040",
    },
    layout: [
      "2222222222222222",
      "2000000000000002",
      "20aa00aa000aa002",
      "20aa00aa000aa002",
      "20b000b0000c0082",
      "2000000000000002",
      "2000000050000002",
      "200000000b000002",
      "2000000000000002",
      "2000000000000002",
      "20000000e0000002",
      "2222222222222222",
    ],
    npcs: [
      { x: 2, y: 4, line: "この まちの きかいは みんな てづくりさ。やどやで さびを おとしていけ。", lineEn: "All the machines in this town are handmade. Scrub off the rust at the inn." },
      { x: 6, y: 4, line: "はいきこうじょうの おくには あぶない きかいが うごいているらしい。", lineEn: "Deep in the scrapped factory, dangerous machines are said to be running." },
      { x: 9, y: 7, line: "きんぞくの においが すきなんだ。おちついた ばしょだよ、ここは。", lineEn: "I love the smell of metal. This is a calm place, this town." },
    ],
  },
  factory_stage2: {
    id: "factory_stage2",
    shortName: "koujou2",
    name: "べるとコンベアのれっしゃ",
    nameEn: "Conveyor Belt Train",
    encounterRate: 0.17,
    wildLevels: [49, 52],
    wildSpecies: ["sparkun", "karakuribat"],
    prevStage: "factory_stage1",
    nextStage: "factory_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#6a6a7a",
      accent: "#7a7a8a",
      bushFill: "#4a4a56",
      bushStroke: "#8a8a9a",
      treeLeaf: "#2b2b38",
      treeFruit: "#f0d040",
    },
    layout: [
      "2222222222222222",
      "2700100000000062",
      "2002010002000002",
      "2002002202002002",
      "2012000100002202",
      "2002000000000002",
      "2002000000031002",
      "2000022000000102",
      "2010000200220002",
      "2010000000020002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t22", money: 2000, x: 7, y: 7, trap: true }],
    groundItems: [{ id: "i22", itemId: "premiumBait", x: 9, y: 4 }],
  },
  factory_cave: {
    id: "factory_cave",
    shortName: "chika",
    name: "ちかはいかんろ",
    nameEn: "Underground Pipeway",
    encounterRate: 0.18,
    wildLevels: [51, 54],
    wildSpecies: ["karakuribat", "paipon"],
    prevStage: "factory_stage2",
    nextStage: "factory_stage3",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#4a5a3a",
      accent: "#5a6a4a",
      bushFill: "#3a4a2e",
      bushStroke: "#8a9a6a",
      treeLeaf: "#2b3520",
      treeFruit: "#f0d040",
    },
    layout: [
      "2222222222222222",
      "2701000000000062",
      "2002000022000002",
      "2002022020020002",
      "2002000100022002",
      "2000000000000002",
      "2020022002200002",
      "2020002002002202",
      "2000000000000202",
      "2000100000000002",
      "2000000010000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t23", money: 2100, x: 8, y: 7, trap: true }],
    groundItems: [{ id: "i23", itemId: "atkSeed", x: 10, y: 8 }],
  },
  factory_stage3: {
    id: "factory_stage3",
    shortName: "koujou3",
    name: "コウジョウヌシのひろば",
    nameEn: "Factory Lord's Clearing",
    encounterRate: 0.18,
    wildLevels: [52, 55],
    wildSpecies: ["kandenkko", "nejimaru", "hagurumaru", "sparkun", "karakuribat", "paipon"],
    prevStage: "factory_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#8a8a9a",
      accent: "#9a9aaa",
      bushFill: "#7a7a8a",
      bushStroke: "#6a6a7a",
      treeLeaf: "#4a4a56",
      treeFruit: "#f0d040",
    },
    layout: [
      "2222222222222222",
      "2700000000000002",
      "2002022000002002",
      "2002000020002202",
      "2002000022000002",
      "2100000000000002",
      "2000022010030002",
      "2010020002010002",
      "2010000002000002",
      "2000000100000002",
      "2000000000000402",
      "2222222222222222",
    ],
    bossId: "koujouNushi",
    bossLevel: 50,
    treasures: [{ id: "t24", money: 2200, x: 6, y: 7, trap: true }],
    groundItems: [{ id: "i24", itemId: "defSeed", x: 12, y: 5 }],
  },
  castle_stage1: {
    id: "castle_stage1",
    shortName: "majou1",
    name: "まおうじょうのおおもん",
    nameEn: "Demon Castle Gate",
    encounterRate: 0.16,
    wildLevels: [53, 56],
    wildSpecies: ["obakenoko", "yamikko", "akumakko", "kokushou", "kuroguri"],
    prevStage: "factory_stage3",
    nextStage: "castle_stage2",
    townStage: "castle_town1",
    spawns: {
      start: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      fromTown: { x: 4, y: 9 },
    },
    palette: {
      ground: "#2b1a3a",
      accent: "#3a2450",
      bushFill: "#1a1024",
      bushStroke: "#5a3a7a",
      treeLeaf: "#150c20",
      treeFruit: "#e04a4a",
    },
    layout: [
      "2222222222222222",
      "2000000000000062",
      "2002200002200002",
      "2002002002002102",
      "2100002200002002",
      "2000000000000002",
      "2010000000030002",
      "2000200000000002",
      "2000220010000002",
      "2000000200000002",
      "2000d00000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t25", money: 2300, x: 8, y: 6, trap: true }],
    groundItems: [{ id: "i25", itemId: "premiumBait", x: 11, y: 4 }],
  },
  castle_town1: {
    id: "castle_town1",
    shortName: "majouMachi",
    name: "まおうじょうかまち",
    nameEn: "Demon Castle Town",
    encounterRate: 0,
    townExitStage: "castle_stage1",
    spawns: {
      start: { x: 8, y: 9 },
      fromField: { x: 8, y: 9 },
    },
    palette: {
      ground: "#4a3a5a",
      accent: "#5a4a6a",
      bushFill: "#1a1024",
      bushStroke: "#5a3a7a",
      treeLeaf: "#150c20",
      treeFruit: "#e04a4a",
    },
    layout: [
      "2222222222222222",
      "2000000000000002",
      "20aa00aa000aa002",
      "20aa00aa000aa002",
      "20b000b0000c0082",
      "2000000000000002",
      "2000000050000002",
      "200000000b000002",
      "2000000000000002",
      "2000000000000002",
      "20000000e0000002",
      "2222222222222222",
    ],
    npcs: [
      { x: 2, y: 4, line: "この まちすら いつ ほろぶか わからない…おだやかな うちに やすんでいけ。", lineEn: "No one knows when even this town might fall... rest while it's still peaceful." },
      { x: 6, y: 4, line: "まおうの ちからは まちの すみずみまで しみついている。ゆだんするな。", lineEn: "The Demon Lord's power has seeped into every corner of this town. Don't let your guard down." },
      { x: 9, y: 7, line: "ここが さいごの やすらぎの ばしょかもしれない。", lineEn: "This might be the last place of peace." },
    ],
  },
  castle_stage2: {
    id: "castle_stage2",
    shortName: "majou2",
    name: "たましいのかいだん",
    nameEn: "Stairway of Souls",
    encounterRate: 0.17,
    wildLevels: [55, 58],
    wildSpecies: ["kumogumo", "kokushou", "yuureiking"],
    prevStage: "castle_stage1",
    nextStage: "castle_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#1a1024",
      accent: "#2b1a3a",
      bushFill: "#150c20",
      bushStroke: "#5a3a7a",
      treeLeaf: "#0a0612",
      treeFruit: "#e04a4a",
    },
    layout: [
      "2222222222222222",
      "2700000100100062",
      "2000220010000102",
      "2000022010210002",
      "2000000000220002",
      "2020000000000002",
      "2122002000030102",
      "2000002000002202",
      "2000200001100202",
      "2000220000000002",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t26", money: 2400, x: 7, y: 7, trap: true }],
    groundItems: [{ id: "i26", itemId: "premiumBait", x: 9, y: 4 }],
  },
  castle_cave: {
    id: "castle_cave",
    shortName: "chika_majou",
    name: "まおうじょうのちかろう",
    nameEn: "Demon Castle Dungeon",
    encounterRate: 0.18,
    wildLevels: [57, 60],
    wildSpecies: ["yuureiking", "noroigumo"],
    prevStage: "castle_stage2",
    nextStage: "castle_stage3",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      fromNext: { x: 13, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#150c20",
      accent: "#1a1024",
      bushFill: "#0a0612",
      bushStroke: "#5a3a7a",
      treeLeaf: "#0d0812",
      treeFruit: "#e04a4a",
    },
    layout: [
      "2222222222222222",
      "2700000001000062",
      "2002202002200002",
      "2000202202002002",
      "2000000000002202",
      "2022000000000002",
      "2002020020000002",
      "2001022002022002",
      "2000000000002002",
      "2000100000000102",
      "2000000000000002",
      "2222222222222222",
    ],
    treasures: [{ id: "t27", money: 2500, x: 8, y: 7, trap: true }],
    groundItems: [{ id: "i27", itemId: "atkSeed", x: 10, y: 8 }],
  },
  castle_stage3: {
    id: "castle_stage3",
    shortName: "majou3",
    name: "マオウのぎょくざ",
    nameEn: "Demon Lord's Throne",
    encounterRate: 0.18,
    wildLevels: [58, 61],
    wildSpecies: ["kumogumo", "obakenoko", "yamikko", "akumakko", "kokushou", "yuureiking", "noroigumo", "kuroguri"],
    prevStage: "castle_cave",
    spawns: {
      fromPrev: { x: 2, y: 1 },
      start: { x: 2, y: 1 },
    },
    palette: {
      ground: "#3a2450",
      accent: "#4a3060",
      bushFill: "#2b1a3a",
      bushStroke: "#5a3a7a",
      treeLeaf: "#1a0c28",
      treeFruit: "#e04a4a",
    },
    layout: [
      "2222222222222222",
      "2700000100000002",
      "2002200002200002",
      "2002002002002002",
      "2000002200002002",
      "2010000000000002",
      "2000200000030002",
      "2001220000000002",
      "2000000211000002",
      "2000000000001002",
      "2000000000000402",
      "2222222222222222",
    ],
    bossId: "maou",
    bossLevel: 55,
    treasures: [{ id: "t28", money: 3000, x: 6, y: 7, trap: true }],
    groundItems: [{ id: "i28", itemId: "defSeed", x: 12, y: 5 }],
  },
};

export function getStage(stageId) {
  return STAGES[stageId] || STAGES[START_STAGE_ID];
}

// ワールドの最終ボス撃破後に次のワールドへ繋がる先。
// 各ボスステージは stage.nextStage をあえて未設定にしている
// （設定するとfield.jsのボス再訪ロジックがstage3専用のChoiceSceneを
// 誤って再表示してしまうため）。そのため、撃破直後のEndingScene遷移を
// 逃した場合（以前のセッションで既に撃破済みなど）でも、ボス部屋に
// 戻れば次のワールドへ進めるように、この対応表を別途用意している。
export const WORLD_TRANSITIONS = {
  reverse_stage3: "sea_stage1",
  sea_stage3: "snow_stage1",
  snow_stage3: "desert_stage1",
  desert_stage3: "factory_stage1",
  factory_stage3: "castle_stage1",
};
